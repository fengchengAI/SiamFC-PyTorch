...
class SiamFCTracker:
    ...
    def update(self, frame):
        ...
        '''
        实际上如果不考虑self.scales，　则disp_response_frame＝　disp_response_input/(2*scale_z)
        scale_z　＝　example_size/s_z;  s_z = sqrt(w * h);  w = w + 0.5(w+h);  h同理
        disp_response_frame　＝　则是在scale_z尺度变换下的disp_response_input转换后的值，除以２是因为距离中心的值
        '''

        # position in frame coordinates
        self.pos += disp_response_frame
        # 为什么self.pos的更新不能直接用disp_response_frame?
        # 因为前面的get_pyramid_instance_image操作会根据上一帧的bound中心进行裁剪的，所以会和训练时一样，response的中间是最大的
        # 但是每一帧之间可能都会有差异，所以会有偏差，即disp_response_interp
